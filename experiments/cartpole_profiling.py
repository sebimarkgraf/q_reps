...
from qreps.valuefunctions import (
    NNQFunction,
    NNValueFunction,
    SimpleQFunction,
    SimpleValueFunction,
...
def create_agent(algo, writer, config, num_obs, num_act):
    ...
    FEAT_DIM = num_obs
    feature_fn = IdentityFeature()

    if algo == "reps":
        value_function = NNValueFunction(obs_dim=FEAT_DIM, feature_fn=feature_fn)
        return REPS(
            policy=policy,
...
def main():
    timestamp = time.time()
    for algo in ["qreps", "qreps_nonparametric"]:
        for it in range(5):
            set_seed(it)
            print(f"Runing {algo}")
